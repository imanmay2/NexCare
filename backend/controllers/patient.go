...
func GetHealthSummary(ctx *gin.Context){
    ...
    userID:=ctx.GetString("userID")
    q1:=` select id,created_at,blood_type,allergies,insaurance,contacts,family_history,surgical_history,current_medications,medical_conditions,menstrual_history,imp from health_record where p_id=$1 `
    ...
    for rows.Next(){
        ...
        rows.Scan(&summ.Id,&summ.Blood_Type,&summ.Allergies,&summ.Insaurance,&summ.Contact,&summ.Family_History,&summ.Surgical_History,&summ.Current_Medication,&summ.Medical_Conditions,&summ.Menstrual_History,&summ.Imp,&summ.Created_At)
